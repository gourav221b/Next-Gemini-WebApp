...
export default function Poem() {
    ...
    useLayoutEffect(() => {
        setLoading(true)
        if (getLocalData('poem-data'))
            setData(prev => JSON.parse(getLocalData('poem-data')))
        setLoading(false)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        localStorage.setItem("poem-data", JSON.stringify(temp))
        setContext(_ => "")
        setLoading(_ => false)
    }
    ...
    return (
        <section className="flex flex-col h-[85dvh] w-full max-w-5xl mx-auto ">
            ...
            <div className="container h-[75dvh] overflow-y-scroll w-full flex flex-col gap-y-8 px-2 md:px-4 my-4" ref={containerRef}>
                {/* default message */}
                <div className="flex border-l-4 border-red-500 px-2">At kobikhoj, we give you poems for situations you provide. Enjoy!</div>

                {/* all content */}
                {data?.map((block, idx) => <div className={"flex px-2 text-md lg:text-md " + (block.type == "res" ? "border-l-4 border-red-500 text-left" : "items-end justify-end border-r-4 border-yellow-500 text-right")} key={idx} id={idx.toString()}>
                    {block?.type == "req" ?
            ...
            <form onSubmit={handleSubmit} className="w-full h-[10dvh] flex bg-[#292929]">
                <input className="bg-transparent resize-none w-11/12 p-4 placeholder:text-sm " placeholder="A romantic poem about victoria memorial" onChange={e => setContext(e.target.value)} value={context} />
